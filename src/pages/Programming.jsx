...
function Programming() {
  // programming news data async thunk
  const { data, dataCount, isLoading, errorMessage } = useSelector(
    (state) => state.programmingNews
  );
  ...
  const handlePageClick = ({ selected }) => {
    navigate(`/programming?page=${selected}`);
  };

  useEffect(() => {
    dispatch(fetchProgrammingNews(currentPage));
  ...
  return isLoading ? (
    ...
    <section className="container mx-auto pb-6">
      <h1 className="px-4 py-6 text-3xl font-bold">Programming</h1>
